...
const ALLOWED_ORIGINS = new Set(
  (Deno.env.get("APP_ALLOWED_RETURN_ORIGINS") ?? "https://ecommerce-control-jv.netlify.app")
    .split(",")
...
function corsHeaders(req: Request) {
  ...
  const allowed = ALLOWED_ORIGINS.has(origin) || preview;
  return {
...
function jsonResponse(req: Request, body: unknown, status = 200) {
  return new Response(JSON.stringify(body, null, 2), {
    status,
    headers: { "Content-Type": "application/json", "Cache-Control": "private, no-store", ...corsHeaders(req) }
  });
}
...
function safeShop(shop: any) {
  return {
    ...
    connectionStatus: shop.status,
    connection_status: shop.status,
    lastSyncAt: shop.last_sync_at ?? null,
    last_sync_at: shop.last_sync_at ?? null,
    lastError: shop.last_error ?? null,
    last_error: shop.last_error ?? null,
    credentialSource: "third_party_v3",
    ...
    syncReady: shop.status === "active" && Boolean(shop.authorization_id),
    sync_ready: shop.status === "active" && Boolean(shop.authorization_id)
  };
}

async function membership(userId: string, accountId: string) {
  ...
  if (!data) throw new HttpError(403, "Conta nÃ£o autorizada para este usuÃ¡rio.");
  return data;
}

async function listShops(userId: string, requestedAccountId?: string) {
  const { data: memberships, error: membershipError } = await supabase
    .from("account_members")
    .select("account_id")
    .eq("user_id", userId);
  if (membershipError) throw membershipError;
  const accountIds = (memberships ?? []).map((row) => String(row.account_id));
  if (requestedAccountId && !accountIds.includes(requestedAccountId)) {
    throw new HttpError(403, "Conta nÃ£o autorizada para este usuÃ¡rio.");
  }
  const allowed = requestedAccountId ? [requestedAccountId] : accountIds;
...
async function loadShop(userId: string, accountId: string, connectionId: string): Promise<ShopRow> {
  ...
  if (!connection.authorization_id || connection.status !== "active") {
    throw new HttpError(409, "A loja precisa ser autorizada novamente pelo OAuth v3.");
  }

  const { data: credentials, error: credentialError } = await supabase.rpc(
    "get_shopee_authorization_tokens_v3",
    { p_authorization_id: connection.authorization_id }
  );
  if (credentialError) throw credentialError;
  const credential = (credentials ?? [])[0];
  if (!credential) throw new HttpError(409, "Credenciais OAuth v3 indisponÃ­veis.");
  ...
  return {
    ...
    shop_name: String(connection.shop_name || `Shopee ${connection.external_shop_id}`),
    ...
    token_expire_in: credential.access_expires_at || null,
    auth_expires_at: credential.refresh_expires_at || null,
    partner_id: Number(credential.partner_id)
  } as ShopRow & { partner_id: number };
}
...
async function assertSyncPermission(userId: string, accountId?: string | null) {
  ...
  const data = await membership(userId, accountId);
...
async function loadCredential(shop: ShopRow): Promise<ShopeeCredential> {
  if (!V3_ENABLED) throw new HttpError(503, "A integração Shopee v3 ainda não está habilitada.");
  const partnerId = Number(shop.partner_id || ENV_PARTNER_ID);
  if (!partnerId || !ENV_PARTNER_KEY) {
    throw new HttpError(503, "Credenciais do app Shopee v3 indisponíveis.");
  }
  if (ENV_PARTNER_ID && partnerId !== ENV_PARTNER_ID) {
    throw new HttpError(409, "A autorização não pertence ao app Shopee v3 configurado.");
  }
  return {
    partnerId,
...
async function refreshAccessToken(shop: ShopRow, credential: ShopeeCredential): Promise<string> {
  ...
  const { error } = await supabase.rpc("rotate_shopee_authorization_tokens_v3", {
    ...
    p_access_expires_at: expireIso,
    p_refresh_expires_at: refreshExpireIso
  });
  if (error) throw error;
  ...
  shop.auth_expires_at = refreshExpireIso;
  return accessToken;
}
...
async function handleAction(userId: string, body: Record<string, any>, req: Request) {
  ...
  const connectionId = String(body.connection_id ?? url.searchParams.get("connection_id") ?? "");
  const supportedActions = ["refresh-token", "sync-products", "sync-variations", "sync-catalog", "sync-product-ads", "sync-orders-step", "sync-orders-batch", "sync-escrow-step", "sync-income-overview", "sync-financial"];
  ...
  if (action === "status") {
    ...
    const connection = connectionId
      ...
      : null;
    if (connectionId && !connection) throw new HttpError(404, "Conexão Shopee não encontrada.");
    return {
      ok: true,
      action,
  ...
  if (!supportedActions.includes(action)) {
    if (["sync-ads-daily-step", "sync-ads-balance", "sync-wallet-step", "sync-returns-step"].includes(action)) {
      throw new HttpError(409, "Esta rotina ainda depende da migração do cron legado.");
    }
  ...
  if (action === "refresh-token") {
    await refreshAccessToken(shop, credential);
    result = { tokenExpireIn: shop.token_expire_in, authExpiresAt: shop.auth_expires_at };
    ...
    result = await syncProducts(shop, credential, statuses);
  } else if (action === "sync-variations") {
    result = await syncVariations(shop, credential);
  ...
  } else if (action === "sync-escrow-step") {
    result = await syncEscrowStep(shop, credential, Number(body.batch_size ?? 50));
  ...
  const { error: updateError } = await supabase
    .from("shopee_connections")
    .update({ last_sync_at: completedAt, last_error: null, updated_at: completedAt })
    .eq("id", shop.connection_id)
    .eq("account_id", shop.account_id);
