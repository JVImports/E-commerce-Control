-- Test Partner IDs are valid only in Sandbox. Persist the selected environment
-- on both the authorization and its connections so Sandbox data can never
-- replace or collide with a Live seller connection.
...
begin
  if p_environment not in ('live','sandbox') then
    raise exception 'Invalid Shopee environment';
  end if;
  ...
  where connection.provider='shopee' and connection.environment=p_environment
  ...
  insert into public.shopee_authorizations(
    ...
    auth_id,oauth.account_id,'third_party_v3_'||p_environment,p_environment,'BR',p_partner_id,'active',
  ...
  foreach shop in array p_shop_ids loop
    insert into public.shopee_connections(
      ...
      auth_id,oauth.account_id,'shopee',p_environment,'BR',shop,'Shopee '||shop::text,
      'active',oauth.user_id,now(),false,
      jsonb_build_object('source','third_party_oauth_v3','environment',p_environment),now()
    )
...
revoke all on function public.finalize_shopee_oauth_v3(text,bigint,text,text,text,timestamptz,timestamptz,bigint[])
  from public,anon,authenticated;
grant execute on function public.finalize_shopee_oauth_v3(text,bigint,text,text,text,timestamptz,timestamptz,bigint[])
  to service_role;
